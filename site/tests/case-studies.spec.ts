...
test.describe("case studies", () => {
  for (const entry of caseStudies) {
    test(`renders ${entry.path}`, async ({ page }) => {
      ...
      await expect(
        page.getByRole("link", { name: entry.backLabel }),
      ).toHaveAttribute("href", entry.backHref);
    });
  }
  ...
  test("featured case study source link is correct", async ({ page }) => {
    ...
    await expect(
      page.getByRole("link", { name: "View portfolio source" }),
    ).toHaveAttribute("href", "https://github.com/KanterLabs/portfolio");
    await expect(
      page.getByText(/Build (\d{2}-\d{2}-\d{4}-\d+|unavailable)/),
    ).toBeVisible();
  });

  test("back to selected work returns to homepage projects anchor", async ({
    page,
  }) => {
    await page.goto("/projects/kanterlabs-homelab");
  ...
  test("project source links use canonical repositories", async ({ page }) => {
    ...
    await expect(
      page.getByRole("link", { name: "View Hostlet repo" }),
    ).toHaveAttribute("href", "https://github.com/KanterLabs/hostlet-core");

    await page.goto("/projects/multi-node-portfolio");
    await expect(
      page.getByRole("link", { name: "View portfolio repo" }),
    ).toHaveAttribute("href", "https://github.com/KanterLabs/portfolio");
  });

  test("Sandbox Factory is presented as a bounded private preview", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "Lifecycle, Telemetry, and Cleanup" }),
    ).toBeVisible();
  ...
  test("data center experience reflects current employment", async ({
    page,
  }) => {
  ...
  test("legacy homelab route redirects to the new case study", async ({
    page,
  }) => {
  ...
  test("heading anchors are keyboard reachable and reveal on focus", async ({
    page,
  }) => {
    await page.goto("/projects/kanterlabs-homelab");
    ...
    await expect
      .poll(() => anchor.evaluate((el) => getComputedStyle(el).opacity))
      .toBe("1");
  ...
  test("homelab diagrams expand and close from the keyboard", async ({
    page,
  }) => {
    await page.goto("/projects/kanterlabs-homelab");

    const trigger = page
      .getByRole("button", { name: /Expand diagram:/ })
      .first();
    await trigger.click();
  ...
  test("metadata row has no decorative empty spans and CSS-generated separators", async ({
    page,
  }) => {
    await page.goto("/projects/hostlet");
    ...
    for (const text of texts) {
      expect(
        text.length,
        `case-meta item "${text}" must not be empty`,
      ).toBeGreaterThan(0);
    }

    const secondBeforeContent = await items
      .nth(1)
      .evaluate((el) => getComputedStyle(el, "::before").content);
    expect(secondBeforeContent).toBe('"·"');
  });

  test("the stack row renders middot separators between entries", async ({
    page,
  }) => {
    await page.goto("/projects/hostlet");
  ...
  test(".case-body divider gap is a divider, not another section", async ({
    page,
    isMobile,
  }) => {
    ...
    for (const { width, expectedPadding } of [
      ...
      expect(paddingTop, `.case-body padding-top at ${width}px`).toBe(
        expectedPadding,
      );
      expect(
        paddingTop,
        `.case-body padding-top < margin-top at ${width}px`,
      ).toBeLessThan(marginTop);
    }
  });
  ...
  test("the TOC rail has a left border and the active link carries an accent marker", async ({
    ...
    const borderLeftWidth = await rail.evaluate((el) =>
    ...
    const activeShadow = await activeLink.evaluate(
    ...
    const beforeBg = await hoverLink.evaluate(
      (el) => getComputedStyle(el).backgroundColor,
    );
    await hoverLink.hover();
    await expect(async () => {
      const afterBg = await hoverLink.evaluate(
        (el) => getComputedStyle(el).backgroundColor,
      );
