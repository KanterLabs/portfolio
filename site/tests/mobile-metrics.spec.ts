...
test("mobile homepage metrics and content spot checks", async ({
  page,
}, testInfo) => {
  await page.goto("/");
  ...
  const metrics = await page.evaluate(() => ({
    ...
    heroParagraph: document
      .querySelector("main section > div p:last-of-type")
      ?.textContent?.replace(/\s+/g, " ")
      .trim(),
    contactText: document.getElementById("contact")?.textContent ?? "",
    sectionTops: ["projects", "about", "architecture", "skills", "contact"].map(
      (id) => ({
        id,
        top: Math.round(
  ...
  expect(metrics.heroParagraph).toContain(
    "Data Center Technician at InterServer",
  );
  ...
  expect(metrics.sectionTops[0]?.top).toBeLessThan(
    metrics.sectionTops[1]?.top ?? 0,
  );
  ...
  await testInfo.attach("mobile-homepage-final", {
    path: screenshotPath,
